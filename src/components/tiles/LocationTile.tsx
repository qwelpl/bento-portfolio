  ...
  if (editing) {
    return (
      <div className="p-4 h-full flex flex-col justify-center gap-3">
        ...
        <input className={inp} value={data.state || ''} onChange={e => s('state', e.target.value)} placeholder="State" />
    ...
    <div className="p-5 h-full flex flex-col justify-center gap-1">
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <span className="text-lg font-medium text-white">
          {[data.city, data.state, data.country].filter(Boolean).join(', ') || 'Somewhere on Earth'}
        </span>
      </div>
    </div>
  )
